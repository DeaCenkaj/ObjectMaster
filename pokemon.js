const pokémon = Object.freeze([
    ...
    const pkmntype = pokémon.filter(p => p.types.length>=2);
   // console.log(pkmntype);
   const pkmnname = pokémon.filter(p => p.id>90).map(p=> p.name);
   ...
   const pkmnflying = pokémon.filter(p => p.types[1]==="flying" ).map(p=> p.name);
